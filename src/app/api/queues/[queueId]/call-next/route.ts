...
import { callNextSchema, queueIdSchema } from "@/lib/queue/schemas";
...
export async function POST(request: NextRequest, context: RouteContext): Promise<NextResponse> {
  ...
  const parsedBody = callNextSchema.safeParse(payload ?? {});
  ...
  try {
    const result = queueStore.callNext(queueIdResult.data, parsedBody.data.finishedStatus);

    return NextResponse.json(result);
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to call next ticket." },
      { status: 409 },
    );
  }
}
